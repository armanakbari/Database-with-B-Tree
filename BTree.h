...
void BTree::insert(Node k) {
    if (root == NULL) {
        ...
        if (root->n == 2 * t - 1) {
            ...
            //I built a BTree node in order to save the new input
            //then I connected the root to it and continue the process in recurssive mode
            s->nextPtr[0] = root;
            s->splitChild(0, root);
            int i = 0;
...
void BTreeNode::insertNonFull(Node k) {
    int i = n - 1;
    //we will consider weather our node is leaf or not
    //then we will find the right place in keys to insert the new data and its nextField
    if (leaf == true) {
        ...
        //if its not a leaf we should find our way down to a leaf
        while (i >= 0 && keys[i].data > k.data)
            i--;
        ...
        //we will continue the process in recurssive mode
...
void BTreeNode::removeFromNonLeaf(int idx){
    ...
    if (nextPtr[idx]->n >= t){
        ...
        //checking next slot in nextPtr
    }else if  (nextPtr[idx+1]->n >= t){
        Node succ = getSucc(idx);
        keys[idx] = succ;
        nextPtr[idx+1]->remove(succ);
    }else{
        //now that we are sure there's no room the nextPtr we will merge tham
        merge(idx);
        nextPtr[idx]->remove(k);
    }
    return;
...
void BTreeNode::borrowFromNext(int idx){
    BTreeNode *child = this->nextPtr[idx];
    BTreeNode *sibling = this->nextPtr[idx+1];
    //our intention is to borrow from sibling
    ...
    if (!sibling->leaf){
        //we should rearrange the sibling nextptr we cause we just borrow one from it.
        for(int i=1; i<=sibling->n; ++i)
            sibling->nextPtr[i-1] = sibling->nextPtr[i];
    }
    //updating the parameter after borrowing is done
    child->n += 1;
    sibling->n -= 1;
    return;
}
